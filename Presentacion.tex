...
% -----------------------------------------------------------------------------
% SLIDE: Problemática de Dark Kitchen y Solución con UniversalWare
% -----------------------------------------------------------------------------
\begin{frame}{Problemática en Dark Kitchens y la Solución UniversalWare}
    \begin{columns}[T]
        \begin{column}{0.48\textwidth}
            \structure{\textbf{¿Qué es una Dark Kitchen y por qué necesita una BD?}}
            \begin{itemize}
                \item \textbf{Sin Comedor Físico:} Modelo enfocado $100\%$ en delivery multi-plataforma (UberEats, Rappi, DidiFood).
                \item \textbf{Alta Volatilidad e Inventario:} Requiere sincronización en tiempo real de insumos, recetas compuestas y estados de pedidos.
                \item \textbf{Múltiples Marcas:} Una misma cocina opera varias marcas virtuales; centralizar la información previene discrepancias de stock.
            \end{itemize}
        \end{column}

        \begin{column}{0.48\textwidth}
            \structure{\textbf{¿Por qué UniversalWare vs. Stack SQL Convencional?}}
            \begin{itemize}
                \item \textbf{Complejidad de ORMs Tradicionales:} Las infraestructuras pesadas (Spring, Django, etc.) consumen excesivos recursos en hardware limitado.
                \item \textbf{API SQL Ligera y Portable:} UniversalWare provee integración directa vía \texttt{cmariadb} en C y LuaJIT, eliminando capas intermedias de abstracción.
                \item \textbf{Baja Latencia en Picos de Demanda:} Mantiene un espacio de memoria mínimo y alta velocidad I/O ideal para operaciones pequeñas o medianas.
            \end{itemize}
        \end{column}
    \end{columns}
\end{frame}
...
% -----------------------------------------------------------------------------
% SLIDE: Comparativa de Eficiencia: Java (JVM) vs. UniversalWare (C + LuaJIT)
% -----------------------------------------------------------------------------
\begin{frame}{Eficiencia en Hardware Limitado: C + LuaJIT vs. Java}
    \begin{columns}[T]
        \begin{column}{0.48\textwidth}
            \structure{\textbf{El Problema de Java en Hardware Limitado}}
            \begin{itemize}
                \item \textbf{Carga Inicial (JVM Warm-up):} Requiere inicializar la máquina virtual en memoria antes de ejecutar lógica, consumiendo recursos críticos.
                \item \textbf{Picos por Garbage Collector:} La recolección de basura genera picos de CPU del $100\%$ en procesadores de gama baja.
                \item \textbf{Overhead por Conexión:} Las bibliotecas tradicionales (JDBC) y contenedores demandan un margen de RAM elevado.
            \end{itemize}
        \end{column}

        \begin{column}{0.48\textwidth}
            \structure{\textbf{Ventaja Operativa de UniversalWare}}
            \begin{itemize}
                \item \textbf{Consumo Mínimo de Memoria:} El runtime de LuaJIT junto a extensiones en C opera en un rango de \textbf{1 a 5 MB} de RAM.
                \item \textbf{Control Directo de Memoria:} Uso de buffers en C para red y persistencia sin pausas no deterministas.
                \item \textbf{Despliegue Ligero:} No requiere dependencias pesadas; ideal para computadoras de bajo costo o servidores pequeños.
            \end{itemize}
        \end{column}
    \end{columns}
...
% -----------------------------------------------------------------------------
% SLIDE: Tabla Comparativa Detallada de Recursos
% -----------------------------------------------------------------------------
\begin{frame}{Tabla Comparativa Detallada: Java vs. UniversalWare}
    \centering
    \resizebox{\textwidth}{!}{
    \begin{tabular}{llll}
        \toprule
        \textbf{Métrica de Recurso} & \textbf{Java + Conector JDBC} & \textbf{UniversalWare} & \textbf{Ventaja de UniversalWare} \\
        \midrule
        \textbf{Peso del Driver en Disco} & $\sim 6\text{ MB} - 10\text{ MB}$ (.jar) & $\mathbf{\sim 50\text{ KB} - 150\text{ KB}}$ (.so/.dll) & Reducción drástica de almacenamiento local. \\
        \textbf{Runtime Base en Disco} & $\sim 150\text{ MB} - 300\text{ MB}$ (JRE) & $\mathbf{\sim 2.5\text{ MB}}$ (LuaJIT + C) & Cabe entero en cualquier memoria Flash vieja o SD. \\
        \textbf{Consumo RAM en Reposo} & $\sim 80\text{ MB} - 250\text{ MB}$ & $\mathbf{\sim 3\text{ MB} - 8\text{ MB}}$ & Ideal para PCs con 2GB RAM de punto de venta. \\
        \textbf{RAM por Conexión Activa} & $\sim 1\text{ MB} - 3\text{ MB}$ & $\mathbf{\sim \text{Kilobytes}}$ (Estructura nativa) & El script de API abre y cierra, liberando todo. \\
        \bottomrule
    \end{tabular}
    }
\end{frame}
...
% -----------------------------------------------------------------------------
% SLIDE: Reglas de Negocio (Parte 1: Catálogos, Productos e Inventario)
% -----------------------------------------------------------------------------
\begin{frame}{Reglas de Negocio y Restricciones (1/2)}
    \begin{columns}[T]
        \begin{column}{0.48\textwidth}
            \structure{\textbf{Marcas y Categorías}}
            \begin{itemize}
                \item \textbf{Unicidad:} Nombres únicos (\texttt{UNIQUE}).
                \item \textbf{Integridad:} Bloqueo de borrado con productos activos (\texttt{ON DELETE RESTRICT}) y actualización en cascada.
            \end{itemize}

            \vspace{0.2cm}
            \structure{\textbf{Productos y Precios}}
            \begin{itemize}
                \item \textbf{Asociación Obligatoria:} Requieren marca y categoría válidas (\texttt{NOT NULL}).
                \item \textbf{Precio Válido:} Debe ser estrictamente positivo (\texttt{CHECK (precio\_venta > 0)}).
            \end{itemize}
        \end{column}

        \begin{column}{0.48\textwidth}
            \structure{\textbf{Inventario y Recetas}}
            \begin{itemize}
                \item \textbf{Unidades Estandarizadas:} Solo admite \texttt{'kg'}, \texttt{'gr'} o \texttt{'pieza'} (\texttt{ENUM}).
                \item \textbf{Control de Stock:} Sin valores negativos (\texttt{CHECK (stock\_actual >= 0)}) y umbral mínimo configurado.
                \item \textbf{Composición M:N:} Clave primaria compuesta (\texttt{id\_producto}, \texttt{id\_ingrediente}) para evitar duplicados.
                \item \textbf{Borrado en Cascada:} La eliminación de un producto remueve sus fórmulas asociadas.
            \end{itemize}
        \end{column}
    \end{columns}
\end{frame}
...
% -----------------------------------------------------------------------------
% SLIDE: Reglas de Negocio (Parte 2: Pedidos, Logística y Detalle)
% -----------------------------------------------------------------------------
\begin{frame}{Reglas de Negocio y Restricciones (2/2)}
    \begin{columns}[T]
        \begin{column}{0.48\textwidth}
            \structure{\textbf{Gestión de Pedidos y Logística}}
            \begin{itemize}
                \item \textbf{Plataformas Autorizadas:} Origen limitado a UberEats, Rappi, DidiFood o WebPropia.
                \item \textbf{Flujo de Estados:} Transición entre \texttt{Pendiente}, \texttt{En Preparacion}, \texttt{En Camino}, \texttt{Entregado} y \texttt{Cancelado}.
                \item \textbf{Auditoría Temporal:} Inserción automática de sello de tiempo con \texttt{CURRENT\_TIMESTAMP}.
                \item \textbf{Asociación de Entidades:} Cliente y repartidor obligatorios (\texttt{RESTRICT}).
            \end{itemize}
        \end{column}

        \begin{column}{0.48\textwidth}
            \structure{\textbf{Detalle de Pedidos}}
            \begin{itemize}
                \item \textbf{Estructura Renglón:} Clave primaria compuesta (\texttt{id\_pedido}, \texttt{id\_producto}).
                \item \textbf{Validación Operativa:} Cantidad mayor a cero (\texttt{cantidad > 0}) y congelamiento de precio unitario.
                \item \textbf{Limpieza en Cascada:} Al borrar un pedido cabecera, sus renglones se eliminan automáticamente.
            \end{itemize}
        \end{column}
    \end{columns}
\end{frame}
    ...
    \begin{columns}[T]
        \begin{column}{0.33\textwidth}
            ...
            \begin{itemize}
                ...
                \item Instala LuaRocks y dependencias como \texttt{luasocket, luasec}.
...
import("cmariadb", "chttp", "cjson", "cjob")

local controllers = {
    categorias = require("fetch.categorias"),
    pedidos    = require("fetch.pedidos")
    -- resto de controladores por cada tabla. . .
}
        ...
        if peticion then
            ...
            if controller and controller[metodo] then
                local success, err = pcall(function()
                    ...
                    pcall(
                        controller[metodo], db, cjson,
                        sanitizar, peticion, escape_sql
                    )
                end)
            end
        end
