...
function M.get(db, cjson, sanitizar, peticion, escape_sql)
    local res = assert(db:query(QUERY_METODO_GET))
...
function M.post(db, cjson, sanitizar, peticion, escape_sql)
    ...
    if not id_cliente or not id_repartidor or not total then 
        error("Parámetros numéricos inválidos") 
    end

    local plataforma_origen = escape_sql(body.plataforma_origen or '')
    local fecha = escape_sql(body.fecha_hora or os.date("%Y-%m-%d %H:%M:%S"))
    local estado = escape_sql(body.estado or 'Pendiente')

    local sql = string.format(
        "INSERT INTO pedidos (id_cliente, id_repartidor, plataforma_origen, fecha_hora, estado, total) VALUES (%d, %d, '%s', '%s', '%s', %.2f);",
...
function M.put(db, cjson, sanitizar, peticion, escape_sql)
    ...
    if not id_pedido or not id_cliente or not id_repartidor or not total then 
        error("Parámetros numéricos inválidos") 
    end
    ...
    local sql = string.format(
        "UPDATE pedidos SET id_cliente = %d, id_repartidor = %d, plataforma_origen = '%s', fecha_hora = '%s', estado = '%s', total = %.2f WHERE id_pedido = %d;",
...
function M.delete(db, cjson, sanitizar, peticion, escape_sql)
    local raw_id = peticion.path:match("id_pedido=(%d+)") or peticion.path:match("id=(%d+)")
    local id = tonumber(raw_id)
    
    if not id then 
        error("Parámetro id_pedido faltante o inválido") 
    end
